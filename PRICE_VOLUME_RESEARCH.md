...
# PVE-056 — Intraday observedAt Is Bar-Start Identity, Not Feature-Known Time

## Current snapshot
Intraday snapshot uses:
- observedAt = bar.time;
- sourceBarTimestamp = bar.time;
- barStart = bar.time;
- barEnd = bar.time + 15m;
- sourceFetchedAt = scheduled monitor time.

For the 09:00 bar:
feature values are only complete after 09:15 and normally fetched around 09:16.

Yet observedAt remains 09:00.

## Correct semantics
Current `observedAt` should be interpreted as:
`BAR_START_IDENTITY_TIME`,
not:
`FEATURE_KNOWN_AT`.

## Point-in-time analysis
For knowledge-time joins use:
- featureKnownAt = sourceFetchedAt when valid;
- and require sourceFetchedAt >= barEnd.

Never join a 09:00-bar feature to a 09:05 event as if the completed-bar feature were already known.

Status: INTRADAY_OBSERVED_AT_SEMANTICS_QUALIFIED.


# PVE-057 — After-Market observedAt=13:30 Predates the Actual After-Market Decision/Fetch Time

## Current daily snapshot
Daily snapshot fixes:
`observedAt = marketDateT13:30:00+08:00`.

But the current Formal after-market process runs much later (approximately 23:35 architecture), after multiple official data sources become available.

The snapshot also stores frozen plan context generated by that after-market process.

## Consequence
The daily row must not be interpreted as:
“all daily PV + Formal context was known at 13:30.”

## Available provenance
The source contains:
`sourceFetchedAt = actual insertion/fetch time`.

Thus current v0.1 can preserve point-in-time safety in analysis only if:
- knowledge time is derived from sourceFetchedAt / scan generatedAt;
- observedAt is treated as market-session anchor identity.

Status: DAILY_OBSERVED_AT_NOT_DECISION_KNOWN_AT.


# PVE-058 — Acceptance enteredAt Is Bar-Start Time although State Is Known after Bar Completion

## Current acceptance
`pvAdvanceAcceptance` receives:
`observedAt = bar.time`.

When a state transitions:
`stateHistory.push({state, enteredAt: observedAt, reason})`.

Therefore a transition created from a completed 09:00~09:15 bar gets enteredAt=09:00.
...
- microstructure;
- event timestamps
could falsely place acceptance 15+ minutes earlier than it became knowable.

## Rule
Interpret v0.1:
- enteredAt = source bar start identity.

For causal/as-of joins derive:
- enteredKnownAt = snapshot sourceFetchedAt / barEnd-qualified known time.

Future version should store both explicitly.

Status: ACCEPTANCE_EVENT_TIME_NEEDS_KNOWN_AT_OVERLAY.


# PVE-059 — Snapshot Identity Time and Knowledge Time Should Remain Separate

## Why bar-start identity is useful
Using bar start in snapshotId gives:
- stable logical bar identity;
- deterministic ordering;
- no dependence on retry minute.

That is good.

## Why it is not enough
Research also needs:
“when could the system first have known this completed feature?”

## Future dual-time contract
- eventTime/barStart: market interval identity;
- barEnd: completion boundary;
- featureKnownAt: actual point-in-time availability;
- recordedAt: persistence/audit time.

These four timestamps answer different questions.

## Rule
Do not “fix” snapshot identity by replacing bar-start with fetched time.
Instead add/consume the correct known-at field for temporal inference.

Status: DUAL_TIME_MODEL_FROZEN.


# PVE-060 — Existing sourceFetchedAt Is Useful but Volatile: Use It for Knowledge Time, Exclude It from Semantic Identity

## Two roles conflict in v0.1
`sourceFetchedAt` is valuable because it proves:
- when the feature was actually fetched/known.

But PVE-037 shows it should not determine whether a logical snapshot is semantically identical on retry.

## Correct separation
Use sourceFetchedAt for:
- point-in-time eligibility;
- latency;
- stale-data QA;
- event alignment.

Exclude volatile acquisition time from:
- semantic fingerprint equality,
unless a time change itself changes the underlying feature source semantics.

## This resolves two problems at once
- no look-ahead joins remain conservative;
- legitimate retries do not become semantic mutations.

Status: FETCHED_AT_KNOWN_TIME_NOT_SEMANTIC_IDENTITY.


# PVE-061 — Current v0.1 Can Still Be Analyzed Point-in-Time If Timestamp Semantics Are Explicitly Repaired by Overlay

## Existing fields are sufficient for many rows
Intraday:
- barStart;
- barEnd;
- sourceFetchedAt.
...
Daily:
- marketDate/bar anchor;
- sourceFetchedAt;
- planDate/context.

## Overlay rule
For v0.1 analysis derive:
`featureKnownAt = sourceFetchedAt`.

Require:
- intraday featureKnownAt >= barEnd;
- daily featureKnownAt belongs to the completed after-market run lineage;
- no event/context joined from after featureKnownAt.

## If sourceFetchedAt missing/invalid
Point-in-time status:
`KNOWN_AT_UNVERIFIED`.

## Limitation
This overlay repairs analysis semantics.
