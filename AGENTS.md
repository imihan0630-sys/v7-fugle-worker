...
## Latest real acceptance: 7.5.26

Actions35163664501 succeeded: all5 provisional qualifiers currentQ2 and previousQ1 reported EPS persisted/read back; readonly1873-stock scan has current/prior review counts5, no missing symbols and futureQuarterRequired=false. Existing plans/capital/Cron/3Min records preserved, no plan POST/phone test. Q1 actual columns are currentYTD/currentQuarter/priorYTD/priorQuarter: validate matching duplicate periods and both EPS pairs; do not use second current column as prior year. First25 data acceptance failed safely on that layout;26 fixes it. Official advertised XBRL link may declare big5, decode only its advertised encoding. That retrieved financial statement has not supplied a validated split/restatement/weighted-share evidence parser. Adjusted QoQ and Q4 source remain unresolved; rule11 and real end-to-end rules remain incomplete. Latest REQUIREMENTS_30.md section is authoritative.
