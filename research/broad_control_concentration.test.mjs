...
const rows = [
  ...
  {scan_date:"2026-09-22",symbol:"4444",cohort:"BROAD_CONTROL",pool:"THOUSAND",snapshot:{sector:{name:"電子"}}},
...
assert.equal(x.totalObservedRows, 6);
...
assert.deepEqual(x.datesWithDuplicateKeys, ["2026-09-22"]);
assert.equal(x.eligibleDenominator, "UNKNOWN");
assert.equal(x.venueCoverage, "UNKNOWN");
assert.equal(x.perDate[0].observedRows, 3);
assert.equal(x.perDate[0].effectiveControls, 3);
assert.equal(x.perDate[0].dataQualityWarning, null);
...
assert.equal(x.perDate[1].observedRows, 3);
assert.equal(x.perDate[1].uniqueSymbolKeys, 2);
assert.equal(x.perDate[1].duplicateRowsBeyondFirst, 1);
assert.deepEqual(x.perDate[1].duplicateSymbols, [{symbol:"4444",rows:2}]);
assert.equal(x.perDate[1].dataQualityWarning, "DUPLICATE_SCAN_DATE_SYMBOL");
assert.equal(x.perDate[1].effectiveControls, 1);
assert.equal(x.perDate[1].industryKnown, 1);
assert.equal(x.perDate[1].industryUnknown, 0);
assert.deepEqual(x.perDate[1].poolXIndustry, {GENERAL:{"金融":1}});
