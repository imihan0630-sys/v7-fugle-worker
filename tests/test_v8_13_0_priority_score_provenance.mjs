...
assert.ok(snapshot.includes("marketConsensusScore:toNumber(item?.marketConsensusScore)"));
assert.ok(snapshot.includes("marketConsensusSources:Number(item?.marketConsensusSources||0)"));
assert.ok(snapshot.includes("marketConsensusBonus:toNumber(item?.marketConsensusBonus)||0"));
...
assert.ok(snapshot.includes('definitionVersion:"FORMAL_PRIORITY_SCORE_BASE_V1_PLUS_MARKET_CONSENSUS_7_5_30"'));
assert.ok(snapshot.includes('comparatorVersion:"PRIORITY_RR_CONSENSUS_SETUP_SECTOR_RS_7_5_30"'));
