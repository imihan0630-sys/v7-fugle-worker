...
assert.ok(content.toString('utf8').includes(baseline.version),'Backup content must contain the current deployed version');
...
await writeFile('/tmp/v7-predeploy-backup/metadata.json',JSON.stringify({version:baseline.version,
...
console.log(JSON.stringify({backupVerified:true,version:baseline.version,bytes:content.length,contentType:response.headers.get('content-type'),cronCount:cron.result?.schedules?.length ?? cron.result?.length}));
