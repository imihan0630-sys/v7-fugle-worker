...
## B-184 — Market-consensus provenance/independence falsification (2026-09-27 Asia/Taipei)
- Fresh canonical read started from B-183; main already contained research commit `970e046044a5717d3bc5311e2b56373b11669524`, so this cycle consumed it rather than repeating it.
- Audit of `scripts/apply_v7_5_30.py`: “independent sources” is not provenance-validated. The POST contract only requires caller-supplied source labels to be non-empty, <=100 chars and unique within the request. It does not freeze provider identity, evidence locator/hash, publication/observation timestamp, knownAt/availableAt, or independence-group identity.
- Thus `sourceCount>=2` proves distinct submitted labels, not informational independence. This is a confirmed measurement/provenance risk, not evidence that historical inputs were duplicated or manipulated.
- PIT mutability risk is structural: the same `V7_MARKET_CONSENSUS:{date}` key may be overwritten for a recent trading date, while selection reads the current value for scanDate. A later same-date update plus rerun can change the overlay unless the decision receipt freezes the version actually eligible/used. Current KV must not be used to reconstruct old decision-time consensus.
- `updatedAt` is receipt write time only and does not prove when each source became known. Prospective evidence needs sourceObservedAt, sourcePublishedAt/availableAt, receiptCapturedAt, firstEligibleDecision, immutable receipt/version identity, normalized provider identity and independenceGroup. Missing fields remain UNKNOWN; no retroactive web reconstruction.
- Before outcome calibration, receipts must prove source identity/independence and PIT eligibility. Otherwise apparent source-count alpha could reflect duplicate labels, shared upstream information or late evidence.
- Bias controls carried forward: selection bias, look-ahead/PIT mutability, market-source bias, date clustering, source-family dependence, redundancy with sector/RS/news attention, Factor Zoo, costs, coverage and zero-pick. No outcomes inspected; no historical Shadow fabricated.
- No threshold/+7 cap/comparator/PriorityScore/sizing/A-B/quota/signal/runtime/Formal change. Shared production ingestion/receipt changes are Class B proposal-first; bonus/threshold/ranking/sizing changes are Class C.
- Status: `DATA_QUALITY_BLOCKED / FALSIFICATION_IN_PROGRESS / NOT_OPTIMIZATION_READY`. No FORMAL_OPTIMIZATION_CANDIDATE.
- Exact next: fresh-read canonical; if not superseded, specify a prospective consensus receipt schema with immutable receiptId/version, normalized provider identity, independenceGroup, evidence locator/hash, source timestamps, capturedAt and firstEligibleDecision; classify any shared-runtime hook as Class B before code. Continue independent structural/redundancy audits using existing PIT fields in parallel.
